  ...
  return (
    <ul>
      {tasks.filter(task => task.listId === list.id).length > 0 ?
        tasks.filter(task => task.listId === list.id).map(task => (
          <li
            key={task.id}
          ...
          >
            {task.id !== newTaskId ?
              <CheckBox
                ...
                onChange={e => {
                  let newtasks = [...tasks]
                  newtasks.find(t => t.id === task.id).completed = e.target.checked
                  setTasks(newtasks)
              ...
              />
              : <i className='w-7 h-7 text-3xl leading-5 text-center not-italic relative top-[1px] -left-1 text-slate-300'>&#43;</i>
            }
            <input
              ...
              autoComplete='off'
              className={classNames('ml-4 w-full truncate bg-transparent outline-none  py-[0.5px] border-b border-transparent focus:border-slate-300 group-hover:border-slate-300 peer placeholder:text-slate-300',
        ...
        : <h3 className='text-xl font-medium text-slate-300'>Задачи отсутствуют</h3>
      }
