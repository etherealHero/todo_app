...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3001/lists')
      ...
      .then(data => {
        if (localStorage.getItem('listId') === null) localStorage.setItem('listId', data[0].id)
        setLists(data)
  ...
  const removeList = list => {
    fetch(`http://localhost:3001/lists/${list.id}`, { method: 'DELETE' })
  ...
  const addList = (name, colorId) => {
    ...
    return fetch('http://localhost:3001/lists', {
      ...
      .then((response) => response.json())
      .then(data => { setLists([...lists, data]); setActiveListHandler(data.id) })
  }
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-64 shrink-0 bg-slate-100 px-8 py-16">
        <div className="fixed w-48">
          {lists?.length > 1 ? <ul className="mb-8">
            <li
              onClick={() => {
                setActiveListHandler(0)
                localStorage.setItem('listId', 0)
              }}
              className={`${activeList === 0 ? 'bg-white shadow-md' : ''} hover:bg-white rounded-md hover:shadow-md px-4 py-2 text-xl cursor-pointer transition-all duration-200`}
            >&equiv;
              <span className="text-base ml-2 relative -top-0.5">Все задачи</span>
            </li>
          </ul> : null}
      ...
      <ul className="p-16 w-full">
        {lists?.length === 0 ?
          <h1 className='text-slate-300 text-2xl font-semibold pt-8'>Задачи отсутствуют</h1> :
          lists && colors && tasks &&
          lists
              ...
              >
                ...
                {!activeList ? null :
                  <button
                    ...
                    id='addTaskBtn'
                  >
                    <i className='w-7 h-7 text-3xl leading-6 text-center not-italic mr-4 relative top-[3px]'>&#43;</i>
                    Новая задача
                  </button>
                }
              </li>
